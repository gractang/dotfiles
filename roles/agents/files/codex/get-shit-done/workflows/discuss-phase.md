...
<required_reading>
@$HOME/.codex/get-shit-done/references/domain-probes.md
@$HOME/.codex/get-shit-done/references/gate-prompts.md
@$HOME/.codex/get-shit-done/references/universal-anti-patterns.md
</required_reading>
...
AGENT_SKILLS_ADVISOR=$(node "$HOME/.codex/get-shit-done/bin/gsd-tools.cjs" agent-skills gsd-advisor 2>/dev/null)
```
...
- Read and execute @$HOME/.codex/get-shit-done/workflows/discuss-phase-power.md end-to-end
...
```bash
TODO_MATCHES=$(node "$HOME/.codex/get-shit-done/bin/gsd-tools.cjs" todo match-phase "${PHASE_NUMBER}")
```
...
3. Resolve model for advisor agents:
   ```bash
   ADVISOR_MODEL=$(node "$HOME/.codex/get-shit-done/bin/gsd-tools.cjs" resolve-model gsd-advisor-researcher --raw)
   ```
...
```bash
MAX_PASSES=$(node "$HOME/.codex/get-shit-done/bin/gsd-tools.cjs" config-get workflow.max_discuss_passes 2>/dev/null || echo "3")
```
...
```bash
node "$HOME/.codex/get-shit-done/bin/gsd-tools.cjs" commit "docs(${padded_phase}): capture phase context" --files "${phase_dir}/${padded_phase}-CONTEXT.md" "${phase_dir}/${padded_phase}-DISCUSSION-LOG.md"
```
...
**Full step-by-step instructions:** @$HOME/.codex/get-shit-done/workflows/discuss-phase-power.md
