...
```bash
cp "$HOME/.codex/get-shit-done/USER-PROFILE.md" "$HOME/.codex/get-shit-done/USER-PROFILE.backup.md"
```
...
✓ Stores profile at $HOME/.codex/get-shit-done/USER-PROFILE.md
...
Run session scan:
```bash
SCAN_RESULT=$(node $HOME/.codex/get-shit-done/bin/gsd-tools.cjs scan-sessions --json 2>/dev/null)
```
...
```bash
SAMPLE_RESULT=$(node $HOME/.codex/get-shit-done/bin/gsd-tools.cjs profile-sample --json 2>/dev/null)
```
...
**Get questions:**
```bash
QUESTIONS=$(node $HOME/.codex/get-shit-done/bin/gsd-tools.cjs profile-questionnaire --json 2>/dev/null)
```
...
```bash
ANALYSIS_RESULT=$(node $HOME/.codex/get-shit-done/bin/gsd-tools.cjs profile-questionnaire --answers "$ANSWERS_PATH" --json 2>/dev/null)
```
...
```bash
node $HOME/.codex/get-shit-done/bin/gsd-tools.cjs write-profile --input "$ANALYSIS_PATH" --json 2>/dev/null
```

Display: "✓ Profile written to $HOME/.codex/get-shit-done/USER-PROFILE.md"
...
```bash
node $HOME/.codex/get-shit-done/bin/gsd-tools.cjs generate-dev-preferences --analysis "$ANALYSIS_PATH" --json 2>/dev/null
```

Display: "✓ Generated /gsd-dev-preferences at $HOME/.codex/commands/gsd/dev-preferences.md"

**For AGENTS.md profile section (if selected):**

```bash
node $HOME/.codex/get-shit-done/bin/gsd-tools.cjs generate-claude-profile --analysis "$ANALYSIS_PATH" --json 2>/dev/null
```
...
```bash
node $HOME/.codex/get-shit-done/bin/gsd-tools.cjs generate-claude-profile --analysis "$ANALYSIS_PATH" --global --json 2>/dev/null
```
...
```bash
BACKUP_PATH="$HOME/.codex/get-shit-done/USER-PROFILE.backup.md"
```
...
```
Artifacts:
  ✓ /gsd-dev-preferences   $HOME/.codex/commands/gsd/dev-preferences.md
