...
```
UI_PHASE_FLAG=$(node "$HOME/.codex/get-shit-done/bin/gsd-tools.cjs" config-get workflow.ui_phase --raw 2>/dev/null || echo "true")
...
```bash
CHECKPOINT=$(node "$HOME/.codex/get-shit-done/bin/gsd-tools.cjs" uat render-checkpoint --file "$uat_path" --raw)
...
- Follow @$HOME/.codex/get-shit-done/workflows/diagnose-issues.md
